...
def validPath(G_0, path, minNofEdges, stats):
    ...
    if hasCycles(G_0, path):
        stats[2] += 1
        print("Has a cycle but continuing")
...
def apply(args):
        ...
        for line in rf:
            ...
            if validPath(G_0, path, minNofEdges, stats):
                ...
                for i in range(len(path)-1):
                    ...
                    if n1 in receptors:
                        ...
                        if len(receptors) == 0:
                            ...
                            print("minNofEdges changed to {}.".format(minNofEdges))
                            #This is for when change happens after receptors are out
                            """
                    n2 = path[i+1]
                    if n2 in tfs:
                ...
                if trackflag and len(receptors) == 0 and len(tfs) == 0:
                    ...
                    trackflag = False
                    print("minNofEdges changed to {}.".format(minNofEdges))
                #THIS IS FOR WHEN CHANGE HAPPENS AFTER ALL TFS AND RECEPTORS ADDED
                """
    ...
    print("{} of them had cycles but were accepted.".format(stats[2]))
